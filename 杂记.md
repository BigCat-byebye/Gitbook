...
```
操作对应的网卡配置文件，例如修改网卡enp3f0为eth1
1. mv /etc/sysconfig/network-scripts/ifcfg-enp3f0 /etc/sysconfig/network-scripts/ifcfg-eth1
2. 修改/etc/sysconfig/network-scripts/ifcfg-eth1中参数如下
	a, 注释uuid这一行
	b, 修改NAME为NAME=eth1
	c, 修改DEVICE为DEVICE=eth1
	b，增加一行HWADDR=【eth1的mac地址】
3. 重启主机（注意一定要重启主机）
```
